...
# --------------------------------------------------------------------
datafolder = PATH + 'predictions/alignments/'
plotfolder = PATH + 'plots/exp_' + '{:02d}'.format(args.exp)
plotfolder = plotfolder + '/alignments_wo_recurrence/' if args.remove_rec else plotfolder + '/alignments/'
pklfilename = datafolder + 'exp_' + '{:02d}'.format(args.exp) + '_' + args.type_seq
pklfilename = pklfilename + '_loss_wo_rec.pkl' if args.remove_rec else pklfilename + '_loss.pkl'
...
for idx_cc, cc in enumerate(criterias):
    ...
    for group_idx, df_gg in groups:
        ...
        if idx_cc == 0:
            ...
            fig.savefig(plotfolder + args.type_seq + '_' + title + '.svg')
            plt.close(fig)

    print('------------------------')

fig1.savefig(plotfolder + args.type_seq + '_Average.svg')
#plt.show()