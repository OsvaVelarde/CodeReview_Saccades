...
struct_cfg = {
	...
	'similarity':'cosine',
    'forwardprop': cfgdict['training']['name'] == 'forward'}
...
for epoch in range(initial_epoch, last_epoch):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        ss_predictions, ss_seq_loss, ss_loss = model(seq_patches) # grads!
...
#       encoder_grads = comp_grads(loss, encoder_output, list(encoder.parameters()))

#       for param, grad in zip(encoder.parameters(), encoder_grads):
#           param.grad = grad
